...
public class Scoreboards
{
	...
	public static String[] Fly(String[] content)
	{
	...
	public static boolean FlyScoreboard(Collection<Player> players, String[] lines, Scoreboard board)
	{
		try
		{
			...
			board.getObjective(DisplaySlot.SIDEBAR).setDisplayName(lines[0]);

			for(int i = 1; i < lines.length; i++)
				if(lines[i] != null && board.getObjective(DisplaySlot.SIDEBAR).getScore(lines[i]).getScore() != 16 - i)
				{
					board.getObjective(DisplaySlot.SIDEBAR).getScore(lines[i]).setScore(16 - i);
			...
			for(String entry : board.getEntries())
			{
				...
				{
					if(element != null && element.equals(entry) && board.getObjective(objName).getScore(entry).getScore() == 16 - Arrays.asList(lines).indexOf(element))
					{
						toErase = false;
						break;
					}
	...
	public static boolean FixScoreboard(Player p, String title, HashMap<String, Integer> elements)
	{
		try
		{
	...
	public static boolean FixScoreboard(Collection<Player> players, String title, HashMap<String, Integer> elements)
	{
		for(Player player : players)
			if(!FixScoreboard(player, title, elements))
				return false;

		return true;
	}

	public static boolean FixScoreboard(Collection<Player> players, String title, HashMap<String, Integer> elements, Scoreboard board)
	{
		try
		{
			title = cutRankedTitle(title);
